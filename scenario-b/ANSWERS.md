...
## Task 31 — Load test, আর কোন endpoint সবচেয়ে খারাপ

Load generator: `scenario-b/scripts/loadtest.sh` — `curl` + `xargs -P` দিয়ে
N টা worker একসাথে চালায়, নির্দিষ্ট সময় পর্যন্ত। বাড়তি package লাগে না।
৬ জনের এক VPS, তাই concurrency ১০-এ আর সময় ৬০ সেকেন্ডে বেঁধে রেখেছি।

**যা মাপা হয়েছে (client side, ৬০ সেকেন্ড করে):**

| Endpoint | concurrency | ৬০s-এ কত request | কার্যত |
|---|---|---|---|
| `/api/notes?limit=20` | 10 | **603** | ~10 req/s |
| `/api/stats` | 10 | **325** | ~5 req/s |
| `/api/notes?limit=5000` | 5 | **5** | **~0 req/s — ৫টা worker ৬০ সেকেন্ডে একটা করেই শেষ করতে পেরেছে** |

**সবচেয়ে খারাপ: `/api/notes?limit=5000`।** ৫ জন client ৬০ সেকেন্ড ধরে চাপ দিয়ে
মোট ৫টা response পেয়েছে — মানে একেকটা request-এ প্রায় **৬০ সেকেন্ড**। একই route
`limit=20`-এ সেকেন্ডে ১০টা সামলায়। পার্থক্যটা পুরোপুরি `limit`-এর, কারণ
`/api/notes`-এ N+1 আছে: query সংখ্যা = `limit + 1`। `limit=20` → ২১টা query,
`limit=5000` → **৫০০১টা query** এক request-এ। এই জন্যই "limit-এ কোনো max নাই"
সমস্যাটা N+1-এর সাথে মিলে সবচেয়ে বাজে জোড়া — একা কোনোটাই এত খারাপ না।

**যেটা আশা করিনি — চাপের সময় monitoring নিজেই অন্ধ হয়ে গেল।**
Load শেষে p95 query দিলে `/api/notes` আর `/api/stats` দুইটাতেই `NaN` আসল, অথচ
`/healthz` (0.0999s) আর `/metrics` (1.0249s) ঠিকই মান দিল। window ফাঁকা থাকলে
**সবগুলো** NaN হত, তাই ওটা কারণ না। আসল সূত্র পাশের line-এ:

```
max_over_time(http_requests_in_flight[5m])  =  1
```

১০টা worker চললে in-flight কখনো ১ হতে পারে না। মানে Prometheus **app ব্যস্ত
থাকা অবস্থায় একবারও scrape করতে পারেনি** — শুধু ফাঁকা মুহূর্তের নমুনা পেয়েছে।
সাথে মিলিয়ে দেখলে `/metrics`-এর নিজের p95 ০.৩২s থেকে বেড়ে **১.০২s** হয়েছে।

কারণ: `scrape_timeout`-এর default হলো `scrape_interval` = ৫s। ২১ query (বা ৫০০১
query) চালানো handler Node-এর event loop আটকে রাখে, তাই `/metrics` সময়মতো
উত্তর দিতে পারে না, scrape fail হয়, ওই series-এর নমুনা পড়া বন্ধ হয়ে যায় —
আর `rate()`-এর জন্য window-এ অন্তত দুইটা নমুনা লাগে, তাই `NaN`।

*যাচাই করার কমান্ড (Prometheus-এ ১৫ দিনের data আছে):*
`query_range` দিয়ে load-এর সময়টার `up{job="notes-api"}` আর
`scrape_duration_seconds{job="notes-api"}` টেনে দেখলে gap/0 ধরা পড়ে।

**শিক্ষা:** in-flight আর p95 **load চলাকালীন** আলাদা terminal থেকে দেখতে হয়,
পরে না। আর `prometheus.yml`-এ `scrape_timeout` আলাদা করে (যেমন `3s`) দেওয়া উচিত,
default-এর ভরসায় থাকলে ঠিক যখন data সবচেয়ে দরকার তখনই data থাকে না।

**যেটা করিনি:** `/api/search` (index ছাড়া `LIKE '%..%'`) এ load দেইনি — উপরের
তিনটা run-এই server-এর অবস্থা যা বোঝার বুঝে গেছি, আর shared VPS-এ বাড়তি চাপ
দিতে চাইনি। ওটা যে ধীর সেটা query আকারেই স্পষ্ট: `body`-তে index নাই, তাই
৩০,০০০ row-এর tenant-এ পুরো table scan।
